...
module Turtle
  ...
  Path = {root: ".turtle", db: ".turtle/database.yml", paths: ".turtle/paths"}
  ...
  def self.root
    ...
    (path.size + 1).times do
      if Turtle.init? (sub = File.join('/', *path))
        return sub
      end
      path.pop
    end
  ...
  class Command
    ...
    def initialize *all
      ...
      @options = options || {}
      @db = Turtle.init?? Database.new(File.join(Turtle.root, Path[:db])) : Database.new
    end

    def run
      if Commands.include? @command
        if Turtle.init? or Initializers.include? @command
          send(@command, *(@args.length == 1 ? @args.first : @args)).tap do |result|
           ...
           err "'turtle' is not initialized here, please run `turtle init`"
        end
      else
        err "#{@command} is not a valid command."
      end
    rescue Database::EntityNotFound
      err "the turtle wasn't found"
    end
    ...
    def init
      unless Turtle.init
        err "'turtle' has already been initialized here"
      else
        true
      end
    end
    ...
    def save
      File.open(File.join(Turtle.root, Path[:db]), 'w') {|f| f.write @db.to_yaml }
    end
